...
int is_bittware_card(FT_DEVICE_LIST_INFO_NODE* device_node)
{
    ...
    if (serial.substr(0, 4) == std::string("ACVP"))
    {
        // Achronix Card
        // needs channel B
        // FIXME Need to turn on channel C GPIO How???
        if (descr.back() == 'B')
            return 1;
        return 0;
    }
    if (serial.substr(0, 3) == std::string("BW"))
    {
        // Bittware Card
        // needs channel A
        if (descr.back() == 'A')
...
int SearchBlasters(int port_num, char* pblaster_name, int blaster_name_sz)
{
    ...
    printd("Found %d devices\n", dwNumDevs);

    memset(g_device_node, 0, sizeof(g_device_node));
    ftStatus = g_pGetDeviceInfoList(g_device_node, &dwNumDevs);
    for(i = 0; i < dwNumDevs; i++)
    {
        printd("%d>%08X %08X %08X %08X *%s* *%s*\n", i,
...
int ftdi_blaster::configureMpsse()
{
    ...
    ftStatus = g_pWrite(m_ftHandle, byOutputBuffer, dwNumBytesToSend, &dwNumBytesSent);
    do
    {
...
unsigned int ftdi_blaster::read_pass_jtagsrv(unsigned int num_bytes, unsigned char* rbufn)
{
    ...
    //Read out the data from input buffer
    ftStatus = g_pRead( m_ftHandle, m_rbuf, num_bytes, &dwNumBytesToRead );
    if (ftStatus != FT_OK)
        return ftStatus;
    ...
    {
        int nbits = rbufn[i];
        unsigned char b = m_rbuf[i];
        b = b >> (8 - nbits);
        m_jbuf[curr / 8] |= (b << (curr & 7)) & 0xFF;
        m_jbuf[curr / 8 + 1] |= (b >> (8 - (curr & 7))) & 0xFF;
        m_jbuf[curr / 8 + 2] = 0;
        curr += nbits;
    }
    ...
    {
        jtagsrvi.jtagsrv_pass_data((void*)m_jtagsrv_context, m_jbuf, curr);
        printd("jtagsrv_pass_data, numbits: %d Chk %08X Data: %02x %02x %02x %02x %02x %02x\n",
               curr, checkSum(m_jbuf,curr),
               m_jbuf[0], m_jbuf[1], m_jbuf[2], m_jbuf[3], m_jbuf[4], m_jbuf[5]);
    }
    return FT_OK;
}
...
unsigned int ftdi_blaster::flush_passive_serial()
{
    ...
    if (numbytes > 0)
    {
        ...
        m_sbuf[idx++] = (unsigned char)(numbytes & 0xFF);
        m_sbuf[idx++] = (unsigned char)((numbytes >> 8) & 0xFF);
        numbytes++;
    ...
    if (numbits > 0)
    {
        ...
        m_sbuf[idx++] = (unsigned char)(numbits);
        b =  m_tdi[i*8+0]     |
            (m_tdi[i*8+1]<<1) |
            (m_tdi[i*8+2]<<2) |
            (m_tdi[i*8+3]<<3) |
    ...
    ftStatus = g_pWrite(m_ftHandle, m_sbuf, idx, &dwNumBytesSent);
...
unsigned int ftdi_blaster::write_read_as_buffer(unsigned int wr_len, unsigned int rd_len, unsigned int bitsidx, unsigned int need_read)
{
    ...
    ftStatus = g_pWrite(m_ftHandle, m_sbuf, wr_len, &dwNumBytesSent);
...
unsigned int ftdi_blaster::write_jtag_stream_as( struct jtag_task* jt )
{
    ...
    for( unsigned int i=0; i<jt->wr_idx; i++ )
    {
        ...
        // ADBUS6 nCE output 1 0
        // ADBUS7 nCS output 1 0
        port_val = m_last_bits_flags & 0xf0;
        if(tms)
            port_val |= 0x08;
        if(tdi)
            port_val |= 0x02;
        ...
        m_sbuf[dwNumBytesToSend++] = 0x80;
        m_sbuf[dwNumBytesToSend++] = port_val;
        m_sbuf[dwNumBytesToSend++] = 0xCB; //direction

        m_sbuf[dwNumBytesToSend++] = 0x80;
        m_sbuf[dwNumBytesToSend++] = port_val | 0x01; //clk pos edge
        m_sbuf[dwNumBytesToSend++] = 0xCB; //direction

        if( jt->need_tdo )
            m_sbuf[dwNumBytesToSend++] = 0x81; //read port bits

        m_sbuf[dwNumBytesToSend++] = 0x80;
        m_sbuf[dwNumBytesToSend++] = port_val; //clk neg edge
        m_sbuf[dwNumBytesToSend++] = 0xCB; //direction
        ...
        if( jt->need_tdo )
            m_sbuf[dwNumBytesToSend++] = 0x87; //force fast reply
    ...
    {
        if (jtagsrvi.jtagsrv_pass_data)
            jtagsrvi.jtagsrv_pass_data(m_jtagsrv_context, (m_rbuf), j);
    }
...
unsigned int ftdi_blaster::write_jtag_stream( struct jtag_task* jt )
{
    ...
    for (unsigned int i = 0; i < jt->wr_idx;)
    {
        ...
        if (can_optimize)
        {
            m_sbuf[wr_idx++] = ftdi_cmd_short;
            m_sbuf[wr_idx++] = BLK_SIZE-1;
            m_sbuf[wr_idx++] = 0;
        ...
        else
        {
            ...
            m_sbuf[wr_idx++] = ftdi_cmd;
            m_sbuf[wr_idx++] = j-1;
            m_sbuf[wr_idx++] = sbyte | (tdi<<7);
            i += j;
            m_rbufn[m_num_rbytes]=j;
            num_bits_sent += j;
            m_num_rbytes++;
        }
        ...
        if(wr_idx>32*1024)
        {
            ...
            //send to chip
            dwNumBytesSent=0;
            ftStatus = g_pWrite(m_ftHandle, m_sbuf, wr_idx, &dwNumBytesSent);
            if(ftStatus)
            { printd("FT_Write err %08X\n",ftStatus); return 0; }

            if(jt->need_tdo)
            {
                ftStatus = read_pass_jtagsrv(m_num_rbytes, m_rbufn);
                if(ftStatus)
                {
            ...
            else
            {
                ...
                {
                    printd("jtagsrv_report: bits %d bits in que %d\n", num_bits_sent, m_num_bits_in_queue);
                    jtagsrvi.jtagsrv_report((void*)m_jtagsrv_context, num_bits_sent, m_num_bits_in_queue);
                }
            }
...
#define FLAG_BIT_NCE 0x10
    ...
    m_last_bits_flags = wrflags;
    m_last_bits_flags_org = flags;
